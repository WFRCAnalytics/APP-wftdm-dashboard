...
- [X] T023 [P] Check `docs/GRAMMAR.md` for any text presenting `scenario:` pinning as the demo/authoring norm; if present, update it to describe the dynamic `$scenario` union as the default and pinning as the deliberate exception (one-draw-per-dataset maps, KPI cards, channel-committed charts).
