...
- [ ] T025 [P] [US2] NOT DONE — deferred to its own future session (deliberate scoping decision, confirmed with the user via `AskUserQuestion` after T024's own scope was fully resolved this session). **Real, confirmed finding, superseding the "very likely the same shape as flowmap" framing this file previously carried**: this file's own scope is NOT simply "flowmap's own pattern applied to zonemap" — its User Story 1 alone (choropleth rendering, auto-fit bounds) is tied to `tests/fixtures/geometry/taz.geoparquet`, a SYNTHETIC 8-zone grid (TAZ 100-900) that T011 already deleted ENTIRELY as part of Phase 2's cutover. Confirmed via direct read (lines 1-390 of this file): `ZONEMAP_TITLE`/`AUTO_DOMAIN_TITLE`/`DIFF_TITLE`/`DIFF_UNRESOLVABLE_TITLE`/`BROKEN_TITLE`/`DIFF_BASELINE_TITLE` and every hardcoded value tied to them (zone count `8`, TAZ IDs 100-900, bounds `[-111.95,-111.75]/[40.68,40.78]` — the SYNTHETIC fixture's own invented coordinates, unrelated to any real data despite coincidentally being Utah-shaped; a diverging-scale domain `[-10,30]` with TAZ 300 at the literal midpoint; an auto-computed domain `[-8,25]`; TAZ 800 as the "no data" case; TAZ 900 as the "excluded, metric row with no geometry" case) requires a COMPLETE re-derivation against the real `public/demo-geometry/taz25.geoparquet` (25 real MTC/SF-area zones, TAZ 1-25) + real `vmt_by_home_taz` metric — this is genuine, substantial new-content-authoring work in its own right, before any of T024's own basemap-panel-reuse pattern (`row_basemap_precedence`/`row_bad_basemap`/`row_broken_composition`, all already real and directly reusable the same way T024 reused them) even becomes relevant. `User Story 2`/`User Story 3`/`019-baseline-diff-consumption`/`014-map-navigation-controls`/`027-map-auto-fit-and-reset`/`User Story 4`'s own real scope was NOT re-surveyed this session (budget was spent confirming User Story 1's real blocker) — re-confirm each via a full line-by-line read, matching T024's own discipline, before writing anything; do not assume any of this file's own previously-recorded describe-block survey without re-checking it against the real, current file first.
