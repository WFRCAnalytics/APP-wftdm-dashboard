...
- [ ] T014 [US1] NOT DONE (deferred — 503-line rewrite, needs iterative browser verification) Migrate `tests/integration/dashboardShell.spec.ts` — replace "Summary/Detail/Basemaps" and any `good_scenario`/fixture-panel-title references with the 7 real demo tab names in order (`Summary`, `Person & Households`, `Tour`, `Mode Choice`, `Trip`, `Network`, `Explore`); tab-count assertions become "7 committed / 8 during tests" or named-tab checks; the "value-box shows a number" test asserts a real KPI (`5,000` / `15,212`, `.first()` where the value now appears more than once).
...
- [ ] T024 [P] [US2] Migrate `tests/integration/flowmapPanel.spec.ts` → Network `od_flows` flowmap; assert flow lines render and the base-map host is `tiles.openfreemap.org/styles/positron`; unreachable-basemap / broken-composition / basemap-precedence → `dashboard-8-test.yaml`.
- [ ] T025 [P] [US2] Migrate `tests/integration/zonemapPanel.spec.ts` → Trip "SOV Trip Attractions by Zone" + a Network zonemap; assert 25 zones tiled, `taz25.geoparquet` geometry, initial auto-fit bounds; unreachable-basemap → `dashboard-8-test.yaml`.
