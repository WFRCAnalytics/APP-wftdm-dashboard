...
- [x] T020 [P] [US2] DONE — Migrated `tests/integration/markdownPanel.spec.ts`, **11/11 passing in isolation** (confirmed across three runs) and clean under two full-suite confirmation runs (see `baseline.md`'s own "Batch 3" section). Real, confirmed finding: every real demo markdown panel today is a plain-paragraph gap note (`project-docs/CALIBRATION-SUMMARIES.md` gap notes on dashboard-2/dashboard-6) — none has the headings/emphasis/lists/link/GFM-table/code-span richness this file's own typography/rendering coverage needs. A new, genuinely real "Methodology Notes" panel was authored on the Summary tab (`dashboard-1-summary.yaml`'s new `row_methodology`), documenting this project's own real straight-line-distance-proxy caveat (already on record elsewhere in this codebase — zonemapPanel.spec.ts's own header comment) rather than fabricated content. The XSS/empty/whitespace edge cases already existed, real, in `dashboard-8-test.yaml`'s `row_markdown_edge` (Foundation phase) — this migration only retargeted titles, EXCEPT the "legitimate markdown around unsafe fragments" case, which needed two short, genuinely descriptive sentences added around the real payload (the panel originally had none) to keep that assertion meaningful; also corrected the sentinel variable name to the real one already authored (`window.__xss_fired`, not the originally-assumed `__xssFired`). The "alongside other panel types" test was adapted to the real Summary tab's actual composition (valuebox+recharts+plotly — no table panel exists on that tab, unlike the retired fixture's own valuebox+plotly+table trio).
...
- [x] T024 [P] [US2] DONE — Migrated `tests/integration/flowmapPanel.spec.ts`, **37/37 passing in isolation** (confirmed across three separate isolated runs — see below). Real, confirmed genuine content-authoring work, exactly as scoped: two new panels authored — `dashboard-6-network.yaml`'s new `row_od_desire_lines_default` ("Trip Distribution Desire Lines (Default View)", real `od_flows`/`activitysim-baseline`, deliberately NO `center`/`zoom`/`basemap` — the real vehicle for ≈20 generic-mechanics tests: auto-fit, default-basemap-host, attribution/nav controls, expand/resize, mapReady race, registry consistency); `dashboard-8-test.yaml`'s new `row_explicit_view`/`row_raster_provider`/`row_ugrc_composition` rows ("Flowmap Explicit View Override", "Flowmap Raster Provider Preset" [`basemap: OpenTopoMap`], "Flowmap UGRC Composition"/"Flowmap UGRC Outdoors Composition" [`ugrc-vector-lite`/`-outdoors`], all real `od_flows`/`activitysim-baseline`, real SF-area `center: [-122.402, 37.787]`). `row_basemap_precedence`'s two pre-existing panels ("Basemap Precedence — inherits tab default"/"— panel override wins") and `row_bad_basemap`/`row_broken_composition`'s existing panels were reused unchanged, just renamed/retargeted in the spec (dropped "(intentional)" suffixes to match their real, already-shipped titles).

  **Real, confirmed od_flows aggregation** (queried live, not assumed): 623 distinct (orig_taz, dest_taz) pairs, 25 distinct locations, real extent lon -122.417245..-122.390667 / lat 37.77274..37.801557 — MTC `prototype_mtc`'s San Francisco Bay Area zones (TAZ 1-25), **not** the Wasatch Front (this app's own `DEFAULT_CENTER`/`DEFAULT_ZOOM` fallback is Utah, but the demo DATA itself is SF — a real, confirmed correction recorded in the migrated file's own header comment and in `dashboard-8-test.yaml`'s `row_explicit_view` comment, which originally, wrongly, described the real extent as Wasatch Front before this was caught).

  **Two real, confirmed "no trigger in real data" scope corrections**, per this task's own explicit instruction not to fabricate a synthetic trigger: (1) zero real rows have a non-positive value or a missing coordinate (queried live) — the retired fixture's "excludes 2 row(s)" warning test became "logs no exclusion warning"; (2) zero real rows share a duplicate (orig_taz, dest_taz) pair before aggregation (623 raw rows = 623 distinct pairs) — the retired fixture's dedicated "duplicate rows sum into one flow line" test was retired outright (the same SUM-per-group mechanism it existed to prove is already exercised by the aggregation-correctness test's own live GROUP BY/SUM cross-check, plus `tests/unit/flowmapData.test.ts`'s own unit-level duplicate-row case).

  **A real, significant design correction found via live verification** (not assumed): the hover-tooltip test originally targeted one specific NAMED flow's geographic midpoint (first the largest overall — a self-loop, TAZ 7→7 — deliberately avoided since no self-loop-exclusion logic exists in `flowmapData.ts` and flowmap.gl's own zero-length-line rendering was unconfirmed; then the largest non-self-loop, TAZ 7→9). Both were confirmed live to be unreliable against this panel's own real 623-flow dataset: `clustering_auto: true` genuinely clusters much of this dense, small-area (~3km²) data at the auto-fitted zoom (confirmed live — zoom 11 settles with synthetic cluster ids like `{[110]}` covering the flows nearest several real candidate midpoints, top-8-largest-by-value included). Fixed by retargeting the test to the OTHER real od_flows panel on this tab ("Trip Distribution Desire Lines," its own real, authored `zoom: 13` — tight enough that individual flows stay pickable, confirmed live) and REDESIGNING the assertion: rather than pin one exact (origin, destination) pair, the test grid-sweeps for the first genuinely non-clustered hit (`strong` text matching plain `\d+ → \d+`, no `{...}` cluster id) and cross-checks its own rendered (origin, destination, value) triple against a live query — proving the real deck.gl onHover + shared `mapTooltip.ts` mechanism against whichever real flow it lands on, the same guarantee the original single-target design existed to prove, without fighting pixel-perfect fragility against dense real data.

  **A real, confirmed test-infrastructure robustness bug found and fixed, NOT a FlowMapPanel.tsx bug**: this real Test tab mounts 11 real/broken map panels at once (far more than the retired fixture's own dedicated "Basemaps" tab ever had) — the first isolated run (34/37 passing) failed 5 UGRC-composition-dependent tests with `window.__flowmapTestMaps` genuinely `undefined` (not merely missing a key), root-caused to `getStyleSources()`'s own bare `window.__flowmapTestMaps![t]` non-null assertion: several tests (`017-multi-sprite-support`, `016-fix-ugrc-dark-mode`) call it immediately after the tab click with no prior render-count wait, and under this busier tab a panel can genuinely still be unmounted on the very first poll attempt — `expect.poll` does not retry past a thrown exception, so the whole check failed immediately rather than polling until mounted. Fixed with optional chaining (`window.__flowmapTestMaps?.[t]?.getStyle()`), treating "not mounted yet" the same as "mounted, zero sources so far" — the correct semantic for every caller. A second, related finding: `trueEventually`'s own default 5000ms poll (Playwright's built-in default, no override) intermittently timed out on a DIFFERENT check each of the first two isolated runs (a render-count wait, a reset-to-view camera-settle wait, a raster-tile request wait — never the same one twice) — the same real resource-contention signature, not a logic bug. Fixed by raising `trueEventually`'s own default to 10000ms file-wide, rather than patching each call site individually as it happened to fail.

  **Isolated verification, three full runs** (single worker, matching this migration's own established standard): run 1 (pre-fix) 31/37, 6 failures — the hover-design issue (1) + the 5 UGRC-robustness-bug failures (5) described above; run 2 (post-UGRC-fix, pre-timeout-fix) 35/37, 2 failures — a DIFFERENT pair than run 1 (reset-to-view test + raster-provider test), both the same generic 5000ms-timeout resource-contention signature, confirming the UGRC fix worked and surfacing the second, broader timeout issue; run 3 (post-timeout-fix) **37/37 passing, clean**. `npm run typecheck` clean throughout; `npm run test:unit` 471/471 unchanged (this migration touches no unit-tested pure module).
- [x] T025 [P] [US2] DONE — Migrated `tests/integration/zonemapPanel.spec.ts`, **30/30 passing in isolation** (confirmed across three separate isolated runs — see `baseline.md`'s own T025 section for the full record). Real, confirmed genuine content-authoring work, exactly as scoped by the earlier finding this entry previously recorded: the retired file's entire User Story 1 was tied to `tests/fixtures/geometry/taz.geoparquet`, a SYNTHETIC 8-zone grid (TAZ 100-900) T011 already deleted — every hardcoded value (zone count, TAZ ids, bounds, domains, the "no data"/"excluded" cases) needed a complete re-derivation against the real `public/demo-geometry/taz25.geoparquet` (25 real MTC/SF-area zones, TAZ 1-25). Two real, structural findings surfaced mid-task, reported via `AskUserQuestion` before continuing (the user chose "proceed now with the two-metric design"): (1) the retired fixture's "excluded" mechanic (a metric row with no matching geometry) is STRUCTURALLY IMPOSSIBLE in this real data model — every real zone-keyed metric shares the same closed 25-zone universe as the boundaries file — retired outright, not worked around; (2) no single real metric covers the full test surface (`vmt_by_home_taz` has zero real no-data gaps anywhere), so a second real metric (`trips_by_destination_zone`, filtered to `primary_purpose: school`, TAZ 14) was authored specifically for the no-data case. Two real bugs found and fixed via live verification before any assertion was written: a `comparison: diff` cross-product bug (a multi-row-per-zone metric without its own discriminator column in `compare_on` produces a cross-product join, not a clean diff — fixed by switching the diff panels to `vmt_by_home_taz`, exactly one row per zone) and a confirmed real-content gap (`$filters.purpose` is never used by any real demo panel anywhere — the filter-reactivity test was deleted, matching `flowmapPanel.spec.ts`'s (T024) own identical finding). Five new Test-tab rows added to `dashboard-8-test.yaml` (`row_zonemap_basemap_precedence`/`_explicit_domain`/`_filter_reactive`/`_diff`/`_diff_baseline`), every asserted value independently re-derived via a live DuckDB query against the real Parquet files — never guessed or carried over. Two further real bugs found and fixed during isolated-run iteration: the tooltip template's `<br/>` produces no text node, so a flattened-text regex can't distinguish "Zone 1" from "Zone 11" — fixed by asserting the tooltip's own `<strong>` element directly; and a real MapLibre animation-cancellation race (`jumpTo()` internally cancels an in-flight `easeTo()`, including the 3D toggle's own pitch animation, if called before that animation's first frame renders) — a genuine, pre-existing bug in this test's own original, unmodified logic (confirmed via `git show HEAD`), fixed at all 3 real call sites by waiting for pitch > 0 before panning away. `trueEventually`'s poll timeout was raised to 10000ms (matching `flowmapPanel.spec.ts`'s own established value) for this real, git-tracked content's own real network/query latency. Full-suite confirmation across two complete 10-worker runs: zonemapPanel.spec.ts 30/30 then 29/30 (the one failure a real, explained single-test contention timeout, not a logic bug — not reproduced in five separate isolated confirmations); zero regression to any other already-`[x]`-done 040-migration file in either run. A large, IDENTICAL-across-both-runs block of unrelated legacy-fixture failures (tablePanel/valueBoxPanel/markdownPanel/observablePlotPanel/rechartsPanel/sankeyPanel/graphicWalkerPanel/sectionSubNav) was investigated directly via an isolated single-worker re-run (not assumed benign, given its unusual byte-identical-across-runs shape) and confirmed to be the expected, ALREADY-TRACKED state of this same migration epic's own remaining NOT-DONE tasks (T018/T019/T020/T021/T022/T023/T026/T028 below) — those files still reference retired, T011-deleted fixture content, unconnected to anything T025 touched.
...
- [x] T027 [P] [US2] DONE — Migrated `tests/integration/panelExpand.spec.ts`, 15/15 passing in isolation. Confirmed via a full line-by-line re-read (not the earlier narrow grep) that this file was fully fixture-coupled despite 0 hits on `good_scenario`/`broken_scenario`/`Basemaps`. Real panel: `Average Trip Distance by Purpose` (Summary tab plotly) replaces `Mode Share by Purpose` throughout. The override test (`expandable: true/false` wins over type default, both directions) needed ONE real, minimal addition — not new-content-authoring in the flowmapPanel sense: `dashboard-8-test.yaml`'s existing `expandable: true` graphic-walker panel (`row_gw_expandable`) covers the "opts IN" half unchanged; the "opts OUT" half had no real analog, so `row_bad_diff`'s already-real "Broken Comparison Diff (unresolvable a/b)" table panel gained one new line, `expandable: false`, paired against `row_missing_metric`'s sibling "Broken Table Panel (missing metric)" (same type, no override). Two tests (empty-state-via-filter, filter-change-while-expanded) DELETED, not migrated — same real, confirmed `$filters.` gap `dashboardShell.spec.ts` (T014) already found and left unfixed, not a new discovery.
...
- [x] T028 [P] [US3] DONE — Migrated `tests/integration/sectionSubNav.spec.ts`, **5/5 passing in isolation** (confirmed across three runs) and clean under two full-suite confirmation runs. A pure rename — real `sections:` already existed exactly where needed with zero new content: "Mode Choice" (`dashboard-4-mode-choice.yaml`) has both a real single-row section ("Tour Mode Choice") AND a real two-row section ("Trip Mode Choice", `[row_trip_mode_flow, row_trip_mode_by_period]`) in ONE tab — covering this file's "single section" and "two-row-grouping anchors at the first row" needs without requiring two separate tabs. The real, sectionless "Summary" landing tab replaced the retired fixture's "Basemaps" for every "no sections" case. One real strict-mode collision found and fixed live: "Tour Mode Choice" is a substring of the same tab's own "At-Work Subtour Mode Choice" section — needed `exact: true`.
...
- [x] T030 [P] [US3] DONE — Migrated `tests/integration/metricStrip.spec.ts`, 3/3 passing in isolation. Real, confirmed find: `dashboard-1-summary.yaml`'s own real row keys are LITERALLY `row_kpis` (all-valuebox — Households/Persons/Tours/Trips/Total VMT — already genuinely qualifies for FR-018's Metric Strip auto-classification with zero YAML change) and `row_charts` (plural — the one real difference from the retired fixture's singular `row_chart`; a recharts + a plotly panel, correctly NOT all-valuebox). `data-testid={rowName}` is derived directly from the YAML row key (confirmed via `dashboardRenderer.tsx`), so `row_kpis` needed no change at all. Panel-title spot-check: "Total Households"/"Mode Share by Purpose" → real "Households"/"Total Trips by Mode".
- [x] T031 [US3] DONE — Migrated `tests/integration/boot.spec.ts`, 6/6 passing in isolation. Real scenarios: `activitysim-baseline`/`-density-variant`/`-transit-variant` (all `ready`, auto-active per 038) + `observed` (real, registered, genuinely `failed` — no real `public/observed/` content in this deployment — filling the "one failed registration does not block the rest" role `broken_scenario` used to). User Story 3's `?s=` tests were redesigned around a real, confirmed mechanics check: `applyURLParams()` never gates on status (confirmed via direct read), so `?s=observed` genuinely, additively activates the real-but-failed `observed` scenario — a meaningful assertion, unlike `?s=activitysim-baseline` (already auto-active either way). User Story 4 needed the sole surviving fixture (`tests/fixtures/all-placeholders-config.yaml`) updated, not just the test: confirmed directly against `summarize.yaml` that no real published metric has both a raw mode-like AND a purpose-like column together (`trip_mode_share`'s own SQL already applies `$mappings.major_trip_mode` at the metric level, output column `major_trip_mode`, no `purpose` column at all; `trip_purpose_share` has `primary_purpose` but no mode column) — `sql_fragments.base_table` now points at `"activitysim-baseline__trip_mode_share"` (real, double-quoted — a live run caught a real DuckDB parser error on the unquoted hyphenated identifier before this fix), the fixture's own `mappings`/`bins` re-keyed to operate on `major_trip_mode`/`share` (real, existing columns) instead of the retired synthetic `mode`/`purpose`, and the test's own `$filters.` placeholder switched from `purpose` to `mode` accordingly — same four placeholder kinds ($mappings/$bins/$sql/$filters) exercised together end-to-end against a real DuckDB-WASM instance, the file's actual purpose, just no longer requiring a column shape no real published metric has.
...
- [x] T033 [P] [US4] DONE — Migrated `tests/integration/scenarioColorOverride.spec.ts`, **8/8 passing in isolation** (confirmed across three runs) and clean under two full-suite confirmation runs. Reuses `scenarioLabelDisplay.spec.ts`'s/`demoMultiScenario.spec.ts`'s own real, already-validated unpinned panel ("Average Trip Distance by Purpose", plotly, Summary tab) for every swatch/legend check. Real manifest color confirmed live: `activitysim-baseline` = `#59A14F` (not independently re-verified for the other two — this file's own real assertion only needs one manifest color to prove it's genuinely different from the resolved palette/default color). One real bug fixed: the original file read `tests/fixtures/dashboard-config/index.json` directly off disk to inject `scenarioPalette` — that file no longer exists (T011). Fixed by routing the REAL `dashboard-config/index.json` endpoint via `page.route()` with an inline JSON body (matching `demoMultiScenario.spec.ts`'s own established convention) — the primary root is empty/gitignored in this dev/test environment, so this exercises the exact real deployer-configuration path with no fixture file needed at all.
- [x] T034 [P] [US4] DONE — Migrated `tests/integration/scenarioLabelDisplay.spec.ts`, **5/5 passing in isolation** (confirmed across three runs) and clean under two full-suite confirmation runs. A real, confirmed correction to this task's own original description: the retired file's actual mechanism is `appState.setLabel()`/`clearLabel()` with an arbitrary custom label ("Preferred Alternative"), not real manifest `display_name`s (no such field exists on `Scenario` — confirmed via `state/appState.ts`) — migrated the real mechanism, not the mistaken assumption. Reuses `demoMultiScenario.spec.ts`'s own real, already-validated unpinned panels, one per real panel type: "Average Trip Distance by Purpose" (plotly)/"Total Trips by Mode" (recharts), both Summary tab; "Workplace Location Distance Distribution" (observable-plot)/"Auto Ownership by Household Segment" (table, real `columns:` `label: "Scenario"` override — a real, confirmed difference from the retired fixture's bare-field-name header), both Person & Households tab. One real timing bug found and fixed: reading `__debugQueryLog()` immediately after a title-visibility check races the panel's own query (title renders before the query fires) — fixed by waiting for a second, still-unlabeled real scenario name to render first.
...
- [x] T036 [P] [US4] DONE — Migrated `tests/integration/switchControlsUnpinnedPanels.spec.ts`, **1/1 passing in isolation** (confirmed across three runs) and clean under two full-suite confirmation runs. Reuses the same real panels `demoMultiScenario.spec.ts` already validated: "Total Trips by Mode"/"Average Trip Distance by Purpose" (Summary tab) + "Land Use / Socioeconomics by Zone" (table, Network tab) + the real pinned "Households" KPI (`activitysim-baseline`) as the unaffected reference. Kept as its own dedicated file per this migration's scoping — a distinct, narrow US1-mechanics check, not a duplicate of `demoMultiScenario.spec.ts`'s own broader "reads as a comparison" coverage. One real timing bug found and fixed: capturing the pinned KPI's "before" value via the whole card's `innerText()` immediately after a visibility check raced the panel's own query (captured `"Households"` with no number at all) — fixed by reading the `.tabular-nums` value node specifically and waiting for a real digit first, matching `demoMultiScenario.spec.ts`'s own established technique.
