...
Verified directly against the real, current `src/panels/panelQuery.ts`, `src/layout/types.ts`, `src/panels/ZoneMapPanel.tsx`, `src/panels/zonemapColor.ts`, `src/panels/tableLogic.ts`, and `project-docs/GRAMMAR.md` before writing this spec — the same discipline every prior feature in this project has required, directly answering all three of the user's own "REQUIRED RESEARCH" questions.
...
- **FR-015**: This feature does not alter `project-docs/GRAMMAR.md`'s panel-type grammar for any field OTHER than adding `comparison`/`compare_on` to `plotly`/`table`/`observable-plot` and the `$baseline` sentinel value to `a`/`b` wherever `comparison: diff` already exists — no other panel-type grammar changes.
