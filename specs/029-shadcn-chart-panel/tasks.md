...
- [X] T024 [P] Document `type: recharts` in `docs/GRAMMAR.md`, alongside the existing `type: plotly`/`type: observable-plot`/`type: sankey` sections — full grammar (`chart_type`/`x`/`y`/`series`/`stacked`), and an explicit note on what this panel type does NOT do (Sankey, legend click-to-toggle) pointing readers to `sankey`/`plotly` instead.
