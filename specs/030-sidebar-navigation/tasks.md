...
- [X] T020 [P] [US2] Document `header.full_page` in `docs/GRAMMAR.md` per `contracts/dashboard-grammar.md`'s own corrected example (FR-019's twelfths convention is a separate, US4 doc addition — T032).
...
- [X] T031 [US4] Branch `src/layout/dashboardRenderer.tsx`'s per-row rendering on `isMetricStripRow()` (T030): `true` → `grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-6` (the 200px minimum and `gap-6` anchor both taken directly from `docs/UX-REDESIGN-PROPOSAL.md`'s own research, per research.md §10); `false` → the existing fraction-based `gridTemplateColumns` math, completely unchanged (FR-018/FR-020). See Shared-file caution above.
- [X] T032 [P] [US4] Document the twelfths-based `width:` authoring convention in `docs/GRAMMAR.md` (FR-019) — documentation only, zero change to `dashboardRenderer.tsx`'s existing fraction-to-`fr`-unit math, which already produces this alignment for any two rows sharing a fraction set (research.md §9).
