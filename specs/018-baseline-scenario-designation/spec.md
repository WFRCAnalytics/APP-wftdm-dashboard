...
Verified directly against the real, current `src/state/appState.ts`, `src/services/scenarioDiscovery.ts`, `src/scenario/scenarioManager.ts`, `src/layout/scenarioLoader.tsx`, `src/services/sqlExpander.ts`, `project-docs/GRAMMAR.md`, and `src/panels/panelQuery.ts` before writing this spec — the same "check the real file before assuming its shape" discipline every prior feature in this project has required.
...
- The new placeholder's exact name/syntax (e.g. `$baseline.<metric>`) is confirmed against `project-docs/GRAMMAR.md`'s existing placeholder-naming convention during planning, not fixed in this spec — the functional shape (FR-007–FR-009) is what's authoritative here.
...
- This feature does not add any new panel type, does not change `project-docs/GRAMMAR.md`'s panel-type grammar for `plotly`/`table`/`observable-plot`/`zonemap`/etc., and does not alter 013-zonemap-panel's `comparison: diff` in any way (FR-010).
