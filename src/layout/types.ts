...
/**
 * Any other panel type (table, flowmap, zonemap, sankey, graphic-walker,
 * markdown — all out of scope for this feature, FR-008) still parses as
 * this base shape rather than being dropped or erroring at parse time —
 * the registry lookup (panels/registry.tsx), not the parser, is what
 * surfaces an unrecognized-type error, keeping "parse" and
 * "renderable-by-this-app" as separate concerns.
 */
export interface UnknownPanelConfig extends PanelConfigBase {
  type: string
}

export type PanelConfig = ValueBoxPanelConfig | PlotlyPanelConfig | UnknownPanelConfig
