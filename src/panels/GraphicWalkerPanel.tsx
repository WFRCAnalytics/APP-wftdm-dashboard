...
export function GraphicWalkerPanel({ config }: { config: GraphicWalkerPanelConfig }) {
  ...
  useEffect(() => {
    ...
    ensureRegistered(scenarioScope.map((scenario) => ({ scenario, metric: selectedDataset })))
      ...
      .then((table) => {
        if (cancelled) return
        // No per-row bigint conversion needed here — `services/
        // duckdb.ts#initDuckDB()`'s `query: { castBigIntToDouble: true }`
        // casts every BIGINT column to DOUBLE inside the query engine
        // itself, so a genuine, reported "Computation service" crash
        // this panel once hit (a COUNT(*)-derived column reaching
        // GraphicWalker's own arithmetic as a raw `bigint`) can no
        // longer occur. See PIPELINE.md's own recorded finding — this
        // was the second of three independent panel types found hitting
        // the same root cause before it was fixed once, globally.
