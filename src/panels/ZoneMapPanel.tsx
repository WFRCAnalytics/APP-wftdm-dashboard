...
export function ZoneMapPanel({ config }: { config: ZoneMapPanelConfig }) {
  ...
  useEffect(() => {
    ...
    //
    // The `typeof raw === 'bigint'` branch this comment used to
    // document (031-all-panel-demo-content's own real, previously-
    // undiscovered gap — a COUNT(*)-derived column, e.g.
    // trips_by_destination_zone.trips, arriving as a JS `bigint`
    // primitive, not `number`) is gone: `services/duckdb.ts#initDuckDB()`
    // now sets `query: { castBigIntToDouble: true }` globally, so a
    // BIGINT column is cast to DOUBLE inside the query engine itself,
    // before this row ever exists — no bigint can reach here anymore.
    // This was independently found and ad hoc patched THREE times
    // (this file, GraphicWalkerPanel.tsx, and a Recharts y-value —
    // 040-test-suite-migration's rechartsPanel.spec.ts Batch B) before
    // the shared root cause was fixed once, globally, per
    // PIPELINE.md's own recorded finding.
