          ...
          <defs>
            {seriesKeys.map((key) => (
              <linearGradient key={key} id={gradientId(gradientIdPrefix, key)} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={`var(--color-${sanitizeColorKey(key)})`} stopOpacity={0.8} />
                <stop offset="95%" stopColor={`var(--color-${sanitizeColorKey(key)})`} stopOpacity={0.1} />
              </linearGradient>
            ))}
          </defs>
        )}
        {seriesKeys.map((key, index) => {
          // 040-test-suite-migration Batch B / PIPELINE.md: must reference
          // the SAME sanitizeColorKey(key) chart.tsx's own ChartStyle used
          // to generate this property's name — a real series value (e.g.
          // "Ride Hail") is not guaranteed to already be a valid bare CSS
          // custom-property name fragment.
          const color = `var(--color-${sanitizeColorKey(key)})`
