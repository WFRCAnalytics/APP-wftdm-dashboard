...
**Addendum (`068-column-type-indicators`, same session, follow-up) — a per-column data-type icon, matching gropaul/dash-ui's own real convention.** Fetched `value-icon.tsx` directly (`src/components/relation/common/value-icon.tsx`) — dash-ui shows a leading type glyph per header (before the column name, before its own sort affordance) via a `normalizeArrowType()`/`ValueIcon()` pair reading a live Arrow schema. **The real finding worth reporting plainly, as asked**: dash-ui's own icon choices are ALREADY `lucide-react` (`Hash`/`Text`/`ToggleLeft`/`Calendar`/`Brackets`/`Braces`/`CircleHelp`) — this app's own icon-library constraint needed no adaptation at all for the types that exist here; `Hash`/`Text`/`ToggleLeft`/`CircleHelp` were reused verbatim. Confirmed directly (not assumed) that `TablePanel.tsx` has no Arrow schema available at all — it calls `query()`, not `queryArrow()` — so, unlike dash-ui, the type is inferred from the already-fetched row *values* (`tableLogic.ts`'s new `inferColumnValueType()`), matching this app's own established convention of treating "column type" as value-derived (`sortRows()`'s numeric-vs-locale branch, `formatValue()`'s `typeof` guard) rather than schema-derived. A real survey of every published demo metric found exactly three real column types today (BIGINT ×90, VARCHAR ×43, DOUBLE ×40 — zero BOOLEAN, zero DATE/TIMESTAMP); a live check further found a real DATE column arrives through `query()`'s own conversion as a plain epoch-millisecond `number`, indistinguishable via `typeof` from a genuine numeric metric — so, unlike dash-ui (which has a real Arrow schema to read), a reliable 'date' case cannot be built here without a schema, and is deliberately left out rather than shipped as an unreliable heuristic; dash-ui's own List/Struct/Map cases are also out of scope (this app's SQL layer never returns a nested value). `boolean` is kept despite zero real content using it yet — `typeof 'boolean'` is exact, at negligible cost. Icon placement matches dash-ui's own header order exactly (type icon → label → sort affordance, all inside the same button, `currentColor`-styled with no distinct color of its own) at the Default (`h-4 w-4`) iconography tier. Verified: `npx tsc --noEmit` clean; `npm run test:unit` 507/507 (7 new — `inferColumnValueType()`'s number/string/boolean/bigint-safety/leading-null-skip/all-null/empty-rows cases); `tests/integration/tablePanelOptionB.spec.ts` extended with a dedicated real-icon-class assertion (6/6 passing, twice consecutively) and two pre-existing tests updated for the now-two-svgs-per-header-button reality; live dual-theme screenshots confirmed the icon reads as one integrated, additive header design — no layout disruption to the Section-label typography or the `067`-shipped sort icons.
...
**Addendum (`059-server-side-pagination`, later session) — PART B's query-architecture recommendation is now IMPLEMENTED.** `TablePanel.tsx` gained a real `TableQueryMode` split: `rowCount < 100_000` stays on today's unmodified fetch-everything path (unchanged, byte-for-byte, for every real published table today — none crosses that line); `rowCount >= 100_000` switches to a genuinely new query-driven path — `ROW_NUMBER() OVER (ORDER BY ... )`-wrapped page queries (`panelQuery.ts`'s new `buildTableDrivenPageQuery()`), re-fetched imperatively on sort/search/page-click, never client-side re-sorted/re-filtered/re-sliced. This resolves the offset-vs-keyset tension §5e/§5f left open in a different way than either alternative first proposed: rather than losing "jump to page N"/"jump to last page" (true keyset pagination's real cost), a `ROW_NUMBER()`-numbered synthetic cursor keeps every existing First/Last/page-jump control working unchanged, while still measuring flat (~40–90ms, `059`'s own `research.md` §2) regardless of cursor depth at real large-table scale — sidestepping naive `LIMIT`/`OFFSET`'s own measured depth-dependent slowdown (§5e above) without keyset's own UI cost. Search *does* move from render-then-filter-in-JS to a SQL-side predicate for a query-driven table (one of the two "still open" bullets below, now resolved) — via DuckDB's own `format()` SQL function reproducing `formatValue.ts`'s exact format-string vocabulary, proven to match the same rendered/formatted text a viewer sees, not raw values. `buildComparisonDiffQuery()`'s own composed query needed zero changes — confirmed directly, not assumed (`059`'s own T022): `resolveQueryAndPairs()`'s already-unified ordinary/comparison-diff resolution feeds the identical `sql` string into the new wrapping queries either way. See `specs/059-server-side-pagination/` for the full spec/plan/research/tasks record, including the real, measured 100,000-row threshold and the depth-independence correction made during planning.

---

## 9. What this document does not decide

**Resolved, no longer open**: whether Option A/B/C is adopted (Option B — implemented, §6); whether/how the `formatValue.ts` bugs are fixed (done, §2); whether real, measured latency numbers exist at real WFRC scale for the query-architecture question (done, §5e — a real synthetic dataset, generated and measured live against the production DuckDB-WASM engine, then fully deleted; nothing was committed to the repo); whether query-time pagination is built, and how (done — `059-server-side-pagination`, see §8's addendum above: a `ROW_NUMBER()`-based synthetic-cursor mode, automatic row-count threshold, offset-pagination UI preserved unchanged); whether search moves to a SQL-side predicate for a query-driven table (done — DuckDB's `format()` reproducing `formatValue.ts`'s own format-string vocabulary, §8's addendum).

**Still genuinely open** — real future work, not decided or built by `059`:

- How `TablePanel.tsx`'s pagination UI would need to change, if at all, for a table so large that even a `ROW_NUMBER()`-numbered synthetic cursor's own `COUNT(*)`/mode-detection query becomes non-negligible — not measured or encountered at any scale `059` tested (its own real 250,000-row synthetic table stayed flat and fast).
- Whether an author-configurable override of the automatic 100,000-row threshold is ever needed (`059` ships a single, global, measured constant — no per-panel grammar field).
- Any change to `panelQuery.ts`'s existing exports, the `columns:`/`sort:`/`pagination:`/`searchable:` grammar's existing shape, or any other panel type beyond `table` — `059`'s own scope is `TablePanel.tsx` alone.
