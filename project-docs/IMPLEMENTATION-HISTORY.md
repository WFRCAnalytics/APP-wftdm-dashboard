# Implementation History — WFRC TDM Calibration Dashboard

**This is a verbatim archive of the full, pre-2026-09-17 `CLAUDE.md`** — kept
because `CLAUDE.md` grew past the 150k-char project-instructions limit
(it hit ~425k chars). `CLAUDE.md` itself was rewritten as a short,
current-state-only reference; every detailed per-feature narrative (bugs
found and fixed, design decisions and why, verification numbers, the full
file-by-file tree with inline history comments) was moved here unchanged.

Read this file when you need the "why" behind existing code, a specific
past bug's root cause, or the full numbered `Implementation order` history
(items 1–32). For current stack/conventions/non-negotiables, read
`CLAUDE.md` instead.

---

# CLAUDE.md — WFRC TDM Calibration Dashboard (archived version)
