...
**GitHub Pages demo build — current mechanism (`039-github-pages-restructure`):** the demo dashboard is deployed to `wfrcanalytics.github.io/APP-wftdm-dashboard/` by a **local build committed by hand**, not a CI pipeline. `npm run build:pages` runs `tsc --noEmit && vite build --outDir docs --emptyOutDir` — same `base: '/APP-wftdm-dashboard/'` as the normal build, output into `docs/` instead of `dist/`. GitHub Pages is configured **"Deploy from a branch → `/docs` folder"**, so committing `docs/` publishes it. `docs/.nojekyll` (sourced from `public/.nojekyll`) turns off Jekyll. There is deliberately **no `.github/workflows/` file** — building and committing `docs/` is a manual step. Vite copies `public/` verbatim, so `public/demo-dashboard-config/`, `public/demo-scenarios/`, `public/demo-geometry/`, `public/basemap/`, and `public/coi-serviceworker.js` all land correctly in `docs/`; verified against a real local build served through a static file server at the Pages path (all seven demo tabs load, zero asset 404s). Note the naming clash this creates: **`docs/` currently holds a built app, not documentation** — the internal reference docs moved to **`project-docs/`** (`ARCHITECTURE.md`, `SPEC.md`, `GRAMMAR.md`, `PIPELINE.md`, `CALIBRATION-SUMMARIES.md`, `UX-REDESIGN-PROPOSAL.md`, `BASEMAP-PICKER-PROPOSAL.md`). Once the real dashboard deploys from WFRC's own server, `docs/` is emptied of build output and reverts to a normal documentation source tree (Python package / YAML authoring docs).
...
**`wftdm-dashboard here`** — serves both the file server and a local copy of the embedded app. Intended to require no internet access, works behind a firewall. Analogous to `simwrapper here`.

**Caveat found during `010-flowmap-panel`'s implementation, not previously checked**: this "no internet required" claim is currently overstated by two real, confirmed exceptions. DuckDB-WASM lazy-loads its own `parquet` extension binary (`parquet.duckdb_extension.wasm`) from `extensions.duckdb.org` via a real network request the first time any query touches a Parquet-backed view — confirmed directly via a `page.on('request')` capture in Playwright, not assumed. This is not specific to any one panel type or feature: it happens on *every* query against a Parquet view, in every deployment mode including `wftdm-dashboard here`, and has been true since `001-data-state-layer`'s own first `read_parquet()` call (commit `0a2cd55`) — it was simply never checked against the "no internet required" claim until now. A real fix exists but is not implemented: DuckDB-WASM supports `SET custom_extension_repository = '<url>'` to redirect extension fetches to a self-hosted mirror instead of `extensions.duckdb.org` — the same self-hosting discipline `services/duckdb.ts` already applies to the DuckDB-WASM *engine* binaries themselves (Vite `?url` imports, never a CDN, per Principle II). Extending that to the `parquet` extension (bundling the `.wasm` file alongside `wftdm-dashboard here`'s own static assets and wiring the `custom_extension_repository` redirect) would close this gap, but `services/duckdb.ts` doesn't do this today. Until it does, read "no internet required" as "no internet required except for one small, one-time-per-session fetch of DuckDB-WASM's own `parquet` extension" — not literally none.

**Second exception, found during `013-zonemap-panel`'s implementation**: DuckDB-WASM's `spatial` extension (needed for `zonemap` panels' `ST_GeomFromWKB`/`ST_AsGeoJSON` geometry parsing) is a real, second instance of the exact same gap — and a strictly *worse* one, in one respect: unlike `parquet`, `json`, `icu`, and `autocomplete` (which DuckDB-WASM auto-loads with no explicit call), `spatial` is not bundled/autoloadable at all — it requires an explicit `INSTALL spatial; LOAD spatial;`, run once lazily the first time any `zonemap` panel needs it (`panels/zoneGeometry.ts`'s `ensureSpatialExtensionLoaded()`), which fetches `spatial.duckdb_extension.wasm` from the same `extensions.duckdb.org` host. Confirmed directly, live, via the same `page.on('request')` method as the `parquet` finding above — a real captured request: `https://extensions.duckdb.org/v1.4.3/wasm_eh/spatial.duckdb_extension.wasm`, firing on first page load of any dashboard tab containing a `zonemap` panel, alongside the pre-existing `parquet.duckdb_extension.wasm` request. The same `custom_extension_repository` self-hosting fix described above would close this gap too, extended to a second extension — not implemented here either, for the same reason (`services/duckdb.ts` doesn't do this today for `parquet`, and this feature doesn't newly implement it for `spatial` rather than fix it once for both). Until either is self-hosted, "no internet required" should be read as "no internet required except for one small, one-time-per-session fetch each of DuckDB-WASM's own `parquet` extension, and — if any `zonemap` panel is present — its `spatial` extension too."
...
documented in `project-docs/CALIBRATION-SUMMARIES.md` — income group, auto sufficiency,
...
structure in `project-docs/SPEC.md`'s Navigation model. This count is not pinned
